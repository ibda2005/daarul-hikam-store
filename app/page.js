...
export default function Dashboard() {
  ...
  const prosesPesanan = async () => {
    ...
    // 1. Simpan riwayat pesanan DAN DETAIL KERANJANG ke database Supabase
    const { error } = await supabase.from('pesanan').insert([
      {
        user_id: userData.id,
        total_harga: totalHarga,
        status_pembayaran: 'Belum Lunas',
        detail_pesanan: keranjang // <--- INI TAMBAHANNYA AGAR ADMIN BISA LIHAT
      }
    ]);

    if (error) return alert("Terjadi kesalahan: " + error.message);

    const dataUntukPDF = { nama: userData.nama_lengkap, laqob: userData.laqob, tingkat: userData.tingkat };
    
    // 2. Download Nota PDF untuk Santri
    cetakNota(dataUntukPDF, keranjang, totalHarga);
    
    // 3. Redirect ke WA
    const noWaAdmin = "6285289031817"; // Ganti dengan nomor WA Anda
    const pesan = `Assalamu'alaikum Admin. Saya *${userData.nama_lengkap}* (Laqob: ${userData.laqob}) telah memesan kitab dengan total *Rp ${totalHarga.toLocaleString('id-ID')}*. Berikut saya lampirkan Nota PDF pemesanan saya.`;
    const waLink = `https://wa.me/${noWaAdmin}?text=${encodeURIComponent(pesan)}`;
    
    alert("Pesanan berhasil dicatat! Nota akan diunduh.");
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-green-700 text-white p-6 shadow-md flex flex-col items-center relative">
        <button onClick={handleLogout} className="absolute top-4 right-4 bg-red-600 px-4 py-2 rounded text-sm hover:bg-red-700 transition">Keluar (Logout)</button>
      ...
      <div className="container mx-auto p-6 flex flex-col md:flex-row gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {daftarKitab.map((kitab) => (
              <div key={kitab.id} className="bg-white p-4 rounded-lg shadow border-l-4 border-yellow-500 flex justify-between items-center">
                <div><h3 className="font-semibold text-gray-800">{kitab.nama_kitab}</h3><p className="text-green-600 font-bold">Rp {kitab.harga.toLocaleString('id-ID')}</p></div>
                <button onClick={() => tambahKeKeranjang(kitab)} className="bg-green-600 text-white w-10 h-10 flex items-center justify-center rounded-full hover:bg-green-700 transition font-bold text-xl">+</button>
              </div>
            ))}
          </div>
        </div>
        <div className="md:w-1/3 bg-white p-6 rounded-lg shadow h-fit border-t-4 border-green-700 sticky top-6">
          ...
          {keranjang.length === 0 ? <p className="text-gray-500 italic">Belum ada kitab dipilih.</p> : (
            <ul className="space-y-3 mb-6">
              {keranjang.map((item, idx) => (
                <li key={idx} className="flex justify-between border-b pb-2 text-sm text-gray-700"><span>{item.nama_kitab} (x{item.jumlah})</span><span>Rp {(item.harga * item.jumlah).toLocaleString('id-ID')}</span></li>
              ))}
            </ul>
          )}
          <div className="flex justify-between font-bold text-lg mb-6 text-gray-800 border-t pt-4"><span>Total:</span><span>Rp {totalHarga.toLocaleString('id-ID')}</span></div>
          <button onClick={prosesPesanan} className="w-full bg-yellow-500 text-green-900 font-bold py-3 rounded hover:bg-yellow-400 transition shadow-md">Buat Nota & Chat Admin</button>
        </div>
      </div>
    </div>
  );
