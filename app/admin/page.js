...
export default function AdminDashboard() {
  ...
  const handleCetakNotaAdmin = (pesanan) => {
    if (!pesanan.detail_pesanan || pesanan.detail_pesanan.length === 0) {
      return alert("Maaf, detail kitab untuk pesanan lama ini tidak tersimpan di database.");
    }
    const dataUser = {
    ...
    // PERUBAHAN DI SINI: Tambahkan 'view' di parameter ke-4
              ...
              daftarPesanan.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-green-50 transition">
                    <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{new Date(p.tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</td>
                    <td className="px-4 py-3"><div className="font-bold text-gray-900">{p.profiles?.nama_lengkap}</div><div className="text-xs text-gray-500">{p.profiles?.laqob} (Tingkat {p.profiles?.tingkat})</div></td>
                    <td className="px-4 py-3 font-semibold text-gray-700 whitespace-nowrap">Rp {p.total_harga.toLocaleString('id-ID')}</td>
                    <td className="px-4 py-3 font-semibold text-green-700 whitespace-nowrap">Rp {(p.sudah_bayar || 0).toLocaleString('id-ID')}<div className="w-full bg-gray-200 rounded-full h-1.5 mt-1"><div className="bg-green-600 h-1.5 rounded-full" style={{ width: `${persen}%` }}></div></div></td>
                    <td className="px-4 py-3 font-bold text-red-600 whitespace-nowrap">{sisa <= 0 ? '-' : `Rp ${sisa.toLocaleString('id-ID')}`}</td>
                    <td className="px-4 py-3 text-center"><span className={`px-2 py-1 rounded text-xs font-bold whitespace-nowrap ${p.status_pembayaran === 'Lunas' ? 'bg-green-100 text-green-800' : p.status_pembayaran === 'Cicilan' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>{p.status_pembayaran}</span></td>
                    <td className="px-4 py-3 text-center whitespace-nowrap flex gap-2 justify-center">
                      {p.status_pembayaran !== 'Lunas' && (
                        <button onClick={() => tambahPembayaran(p)} className="bg-blue-600 text-white px-3 py-1 rounded text-xs hover:bg-blue-700 transition shadow">+ Bayar</button>
                      )}
                      
                      {/* Teks Tombol Diubah Menjadi "Lihat Nota" */}
                      {p.detail_pesanan && (
                         <button onClick={() => handleCetakNotaAdmin(p)} className="bg-gray-800 text-white px-3 py-1 rounded text-xs hover:bg-black transition shadow">Lihat Nota</button>
                      )}
                    </td>
                  </tr>
                );
